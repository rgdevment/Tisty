...
fn into_doc(
    ...
    let body = tisty_core::docs::read(&root, which).map_err(|e| room_or(e, lang))?;
...
fn room_or(e: tisty_core::Error, lang: Lang) -> anyhow::Error {
    match e {
        tisty_core::Error::DocumentTooBig { .. } => anyhow::anyhow!("{}", lang.get("doc-no-room")),
