...
describe("a round landing while the window is open", () => {
  ...
  it("stops asking about a document whose body is not here at all", async () => {
    const astray = [
      {
        id: "01E",
        file: "a3f1-0009",
    ...
    const answered = ipc.answer;
    ipc.answer = (cmd, args) => {
      if (cmd === "docs") return Promise.resolve({ folders: [], docs: astray });
      if (cmd === "docs_catch_up") return Promise.resolve(astray);
      return answered(cmd, args);
    };
    render(<App />);
    ...
    await waitFor(() => expect(ipc.calls.some((one) => one.cmd === "docs")).toBe(true));
...
describe("the views nothing else opens", () => {
  ...
  it("takes an offer the Store itself made without leaving the window", async () => {
    const was = ipc.answer;
    ipc.answer = (cmd, args) => {
      if (cmd === "update_ready") {
        return Promise.resolve({
    ...
    expect(screen.queryByText(/open the Store/i)).toBeNull();
  });
  ...
  it("sends whoever the Store does not answer for back to the Store", async () => {
    ...
    expect(await screen.findByText(/open the Store/i)).toBeTruthy();
